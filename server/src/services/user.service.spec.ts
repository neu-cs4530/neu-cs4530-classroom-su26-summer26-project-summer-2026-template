...
import { enforceAuth, updateUser } from "../../src/services/user.service.ts";
...
// user id
describe("updateUser", () => {
  it("should throw if given an invalid user id", () => {
    expect(() => updateUser("fake", { display: "Stacey Fakename" })).toThrow();
  });
});
...
describe("enforceAuth", () => {
  it("should return a user and id on good auth", () => {
    const user = enforceAuth({ username: "user1", password: "pwd1111" });
