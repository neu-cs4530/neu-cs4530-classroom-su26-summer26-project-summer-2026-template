 ...
 */
export const postCreate: RestAPI<GameInfo> = (req, res) => {
  ...
  const game = createGame(user, body.data.payload, new Date());
  res.send(game);
};
 ...
 * object.
 */
export const getById: RestAPI<GameInfo, { id: string }> = (req, res) => {
 ...
 */
export const getList: RestAPI<GameInfo[]> = (req, res) => {
  res.send(getGames());
};
...
export const socketWatch: SocketAPI = (socket) => async (body) => {
  try {
    ...
    const user = enforceAuth(auth);
    const { isPlayer, view, players } = viewGame(gameId, user);
...
export const socketJoinAsPlayer: SocketAPI = (socket, io) => async (body) => {
  try {
    ...
    if (game.players.length === gameServices[game.type].maxPlayers) {
      sendViewUpdates(io, gameId, startGame(gameId, user));
    }
  } catch (err) {
    logSocketError(socket, err);
  }
 ...
 */
export const socketStart: SocketAPI = (socket, io) => (body) => {
  try {
    const { auth, payload: gameId } = withAuth(z.string()).parse(body);
    const user = enforceAuth(auth);
    sendViewUpdates(io, gameId, startGame(gameId, user));
 ...
 */
export const socketMakeMove: SocketAPI = (socket, io) => (body) => {
  try {
    const {
      auth,
      payload: { gameId, move },
    ...
    const user = enforceAuth(auth);
    const viewUpdates = updateGame(gameId, user, move);
