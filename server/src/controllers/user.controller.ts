 ...
 */
export const postLogin: RestAPI<SafeUserInfo> = (req, res) => {
  ...
  const user = checkAuth(userAuth.data);
  if (!user) {
    res.send({ error: "Invalid username or password" });
    return;
  }

  res.send(populateSafeUserInfo(user.userId));
};
 ...
 */
export const postByUsername: RestAPI<SafeUserInfo, { username: string }> = (req, res) => {
  ...
  res.send(updateUser(req.params.username, body.data.payload));
};
 ...
 */
export const postSignup: RestAPI<SafeUserInfo> = (req, res) => {
  const userAuth = zUserAuth.safeParse(req.body);
  if (!userAuth.success) {
    res.status(400).send({ error: "Poorly-formed request" });
    return;
  ...
  res.send(createUser(userAuth.data.username, userAuth.data.password, new Date()));
};
 ...
 */
export const getByUsername: RestAPI<SafeUserInfo, { username: string }> = (req, res) => {
 ...
 */
export const postList: RestAPI<SafeUserInfo[]> = (req, res) => {
  ...
  try {
    users = getUsersByUsername(usernames.data);
  } catch {
