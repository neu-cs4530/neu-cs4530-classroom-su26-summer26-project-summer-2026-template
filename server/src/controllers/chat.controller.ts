...
export const socketJoin: SocketAPI = (socket) => async (body) => {
  try {
    ...
    const chat = forceChatById(chatId, user);
    ...
    socket.to(chatId).emit("chatUserJoined", { chatId, user: populateSafeUserInfo(user.userId) });
  } catch (err) {
...
export const socketLeave: SocketAPI = (socket) => async (body) => {
  try {
    ...
    socket.to(chatId).emit("chatUserLeft", { chatId, user: populateSafeUserInfo(user.userId) });
  } catch (err) {
    logSocketError(socket, err);
  }
};
 ...
 */
export const socketSendMessage: SocketAPI = (socket, io) => (body) => {
  try {
    const {
      auth,
      payload: { chatId, text },
    ...
    const user = enforceAuth(auth);
    const now = new Date();
    const message = createMessage(user, text, now);
    addMessageToChat(chatId, user, message.messageId);
