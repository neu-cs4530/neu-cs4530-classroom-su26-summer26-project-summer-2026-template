...
describe("POST/api/user/:username", () => {
  ...
  it("should update individual parts of a user correctly", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.display).toStrictEqual("New User 1 Display");
    ...
    expect(response.status).toBe(200);
    expect(response.body.display).toStrictEqual("Newer User 1 Display");
  });
});
...
describe("POST /api/user/list", () => {
  ...
  it("accepts valid usernames and returns appropriate responses", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body[0].username).toStrictEqual("user2");
    expect(response.body[1].username).toStrictEqual("user1");
  });
  ...
  it("accepts duplicates and returns users in the order provided", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body[0].username).toStrictEqual("user1");
    expect(response.body[1].username).toStrictEqual("user2");
    expect(response.body[2].username).toStrictEqual("user1");
  });
});
